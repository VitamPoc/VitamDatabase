 ...
 * 
 */
public class DbModelMysql extends org.waarp.common.database.model.DbModelMysql implements DbVitamModel {
	/**
	 * Internal Logger
	 */
	...
	private static final String dropTableMysql = "DROP TABLE IF EXIST ";
	private static final String dropIndexMysql = "DROP INDEX ";
	private static final String cascade = " CASCADE";
	private static final String createTableMysql = "CREATE TABLE IF NOT EXIST ";
	private static final String createIdxMysql = "CREATE INDEX ";
	...
	public void dropTables(DbSession session, DbSchema schema) throws WaarpDatabaseNoDataException,
		...
		for (String tableName : schema.constructionOrder) {
			...
			String action = dropTableMysql + table.name + cascade;
	...
	public void createTables(DbSession session, DbSchema schema) throws WaarpDatabaseNoDataException, WaarpDatabaseNoConnectionException  {
		...
		for (String tableName : schema.constructionOrder) {
			...
			if (table.primaryKeys != null && ! table.primaryKeys.isEmpty()) {
				...
				primary += ") ";
				action = dropIndexMysql + table.name+"_pk";
				System.out.println(action);
				try {
					request.query(action);
				} catch (WaarpDatabaseNoConnectionException e) {
			...
			action = createTableMysql + table.name + "(";
			...
			if (table.indexes != null && ! table.indexes.isEmpty()) {
				for (String idxname : table.indexes.keySet()) {
					...
					action = createIdxMysql+idxname+" ON " + table.name + "(";
	...
	public String getLimitOffset(String action, int start, int limit) {
		...
		if (limit > 0) {
			if (start > 0) {
				return action + " LIMIT " + start + "," + limit; 
			}
			return action + " LIMIT " + limit;
		}
		return result;
	}
