 ...
 * 
 */
public class DbModelH2 extends org.waarp.common.database.model.DbModelH2 implements DbVitamModel {
	/**
	 * Internal Logger
	 */
	...
	private static final String dropTableH2 = "DROP TABLE IF EXISTS ";
	private static final String dropIndexH2 = "DROP INDEX IF EXISTS ";
	private static final String cascade = " CASCADE";
	private static final String createTableH2 = "CREATE TABLE IF NOT EXISTS ";
	private static final String createIdxH2 = "CREATE INDEX IF NOT EXISTS ";
	...
	public void setDefaultTimeOut(DbSession session, long timeout) {
		...
		try {
			request = new DbRequest(session);
			request.query("SET LOCK_TIMEOUT " + timeout);
		} catch (WaarpDatabaseNoConnectionException e) {
			logger.warn(StaticValues.LBL.error_error.get() + e);
			return;
		} catch (WaarpDatabaseSqlException e) {
	...
	public void dropTables(DbSession session, DbSchema schema) throws WaarpDatabaseNoDataException,
		...
		for (String tableName : schema.constructionOrder) {
			...
			String action = dropTableH2 + table.name + cascade;
	...
	public void createTables(DbSession session, DbSchema schema) throws WaarpDatabaseNoDataException, WaarpDatabaseNoConnectionException  {
		...
		for (String tableName : schema.constructionOrder) {
			...
			if (table.primaryKeys != null && ! table.primaryKeys.isEmpty()) {
				...
				primary += ") ";
				action = dropIndexH2 + table.name+"_pk";
				System.out.println(action);
				try {
					request.query(action);
				} catch (WaarpDatabaseNoConnectionException e) {
			...
			action = createTableH2 + table.name + "(";
			for (int i = 0; i < fields.size(); i++) {
				...
				if (field.length > 0) {
					switch (field.type) {
						...
						case Types.TINYINT:
							if (field.length <= 0) {
								action += DBType.BIGINT.constructor;
							} else if (field.length <= 2) {
								action += DBType.TINYINT.constructor;
							} else if (field.length <= 4) {
								action += DBType.SMALLINT.constructor;
							} else if (field.length <= 9) {
								action += DBType.INTEGER.constructor;
							} else {
								action += DBType.BIGINT.constructor;
							}
							break;
						...
						case Types.LONGVARCHAR:
							if (field.length == 0) {
								...
								action += " LONGVARCHAR("+field.length+") ";
							}
							break;
						default:
			...
			if (table.indexes != null && ! table.indexes.isEmpty()) {
				for (String idxname : table.indexes.keySet()) {
					...
					action = createIdxH2+idxname+" ON " + table.name + "(";
	...
	public void createTables(DbSession session) throws WaarpDatabaseNoConnectionException {
		// Do nothing since done in other method
	}
